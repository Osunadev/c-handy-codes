...
int verifAnterior (double *var, int size);
...
int main() {
    ...
    printf("\nTamanyo del sistema de ecuaciones: ");
    scanf("%d",&tam);
    double variables[2][tam]; //variables a utilizar para guardar los valores de a,b,c,...
    ...
    valoresIniciales(&variables[0][0],tam);
    ...
    do {
        ...
        for (i=0; i<tam; i++) {
            ...
            for (j=0; j<tam; j++) {
                ...
                if(j!=i) {
                    eval  += (-1*sistema[i][j])*variables[0][j]; //Multiplicando por los valores dados
                }
            }
            eval /= sistema[i][i]; //Se divide por el coeficiente de la diagonal
            variables[1][i] = eval; //Asignando la evaluacion a la variable que le corresponda
        }
        //Si los valores anteriores son iguales a los actuales
        printf("\n%d)Iteracion \n",iter+1);


        iter++;
        for (i=0; i<tam; i++) printf("%f \t", variables[1][i]);
        

        if (verifAnterior(&variables[0][0],tam)) count++;

         if (count == 8) break;
        //Guardando los valores actuales en los valores iniciales
        for (i=0; i<tam; i++) variables[0][i] = variables[1][i];
        

    } while(!solucionRaices(sistema,&variables[0][0],tam));
...
int verifAnterior (double *var, int size) {
	...
	for (i=0; i<size; i++) {
		if ( abs(*(var+i) - *(var+size+i)) == 0) verif++;
	}
